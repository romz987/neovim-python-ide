...
Задача этого руководства — собрать в одном документе полный, проверенный на практике путь от установки Neovim до получения рабочей Python IDE с отладчиком, линтерами и форматтерами, чтобы не пришлось собирать информацию из десятка разрозненных источников.

Проект состоит из пяти основных этапов:  
1. Установка и настройка NvChad.  
2. Установка инструментов разработчика: treesitter парсеры, lsp-сервер pyright, линтер ruff, форматтер black.  
3. Установка дополнительных инструментов для работы с конфигами и другими важными форматами файлов.  
4. Установка и настройка отладчика (nvim-dap + debugpy).  
...
Репозиторий содержит мои конфигурационные файлы `config/lua`, написанные в соответствии с этим руководством.  
Их можно использовать в качестве примеров или скопировать целиком.  
